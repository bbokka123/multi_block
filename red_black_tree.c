...
Rotate(ElementType Item, Position Parent) {
    ...
    if (strcmp(Item ,Parent->Element) < 0 )
...
void HandleReorient(ElementType Item, RedBlackTree T) {
    ...
    if (P->Color == Red) /* Have to rotate */ {
        GP->Color = Red;
        if (strcmp(Item , GP->Element) != strcmp(Item . P->Element))
...
Insert(ElementType Item, RedBlackTree T) {
    ...
    if (X == NULL)
        FatalError("Out of space!!!");
    X->Element = (char *)malloc(strlen(Item)+1);
    strcpy(X->Element , Item);
    X->Left = X->Right = NullNode;
 
    if (strcmp(Item , P->Element)) /* Attach to its parent */
        P->Left = X;
    else
        P->Right = X;
